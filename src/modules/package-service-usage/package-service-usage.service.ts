...
export class PackageServiceUsageService {
    ...
    async create(createDto: CreatePackageServiceUsageDto) {
        ...
        if (
            ...
            subscription.payment.status !== PaymentStatusType.COMPLETED
        ) {
            throw new BadRequestException(
                `Subscription '${subscriptionId}' has not been paid`,
            );
        }
        ...
        if (!isServiceInPackage) {
            throw new BadRequestException(
                `Service '${serviceId}' is not included in the package`,
            );
        }
        ...
        const quantityLimit =
            subscription.package.maxServicesPerMonth ?? Number.MAX_SAFE_INTEGER;
        ...
        return await this.packageServiceUsageRepository.save(
            packageServiceUsage,
        );
    }
    ...
    async findOne(id: string) {
        const packageServiceUsage =
            await this.packageServiceUsageRepository.findOne({
    ...
    async update(id: string, updateDto: UpdatePackageServiceUsageDto) {
        ...
        this.packageServiceUsageRepository.merge(
            packageServiceUsage,
            updateDto,
        );
        return await this.packageServiceUsageRepository.save(
