    ...
    implements OnGatewayConnection, OnGatewayDisconnect
{
    ...
    async handleAuthenticate(
        ...
        try {
            ...
            this.logger.log(
                `User authenticated: ${user.id} (${user.fullName})`,
            );
        } catch (error) {
    ...
    async notifyAnswerAccepted(answer: any, questionId: string) {
        ...
        if (answer.consultantId) {
            const consultantSocketId = this.connectedUsers.get(
                answer.consultantId,
            );
            if (consultantSocketId) {
                this.server
                    .to(consultantSocketId)
                    .emit('your_answer_accepted', {
