...
export class UserPackageSubscriptionsService {
  ...
  async create(createDto: CreateUserPackageSubscriptionDto) {
    ...
    const packageEntity = await this.packageRepository.findOne({ where: { id: packageId, deletedAt: IsNull(), isActive: true } });
    if (!packageEntity) {
      throw new NotFoundException(`Service package with ID '${packageId}' not found or inactive`);
    }

    // Kiểm tra thanh toán
    const payment = await this.paymentRepository.findOne({ where: { id: paymentId, deletedAt: IsNull(), status: PaymentStatusType.COMPLETED } });
    if (!payment) {
      throw new NotFoundException(`Payment with ID '${paymentId}' not found or not completed`);
    }
  ...
  async update(id: string, updateDto: UpdateUserPackageSubscriptionDto) {
    ...
    if (subscription.status === SubscriptionStatusType.EXPIRED || subscription.status === SubscriptionStatusType.CANCELLED) {
