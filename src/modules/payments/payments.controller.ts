...
export class PaymentsController {
  ...
  async handleCancel(@Query() query: any) {
    ...
    try {
      const payment = await this.paymentsService.handleCancelCallback(orderCode);
  ...
  async handleInvalidCancel() {
    throw new BadRequestException('Yêu cầu hủy không hợp lệ. Vui lòng sử dụng đúng URL /payments/cancel với các tham số query hợp lệ.');
  }
  ...
  @Get(':id')
  findOne(@Param('id', new ParseUUIDPipe({ errorHttpStatusCode: 400, exceptionFactory: () => new BadRequestException('ID phải là UUID hợp lệ') })) id: string) {
    console.log(`Yêu cầu tìm thanh toán với id: ${id}`);
    return this.paymentsService.findOne(id);
  }

  @Patch(':id')
  update(@Param('id', new ParseUUIDPipe({ errorHttpStatusCode: 400, exceptionFactory: () => new BadRequestException('ID phải là UUID hợp lệ') })) id: string, @Body() updatePaymentDto: UpdatePaymentDto) {
    console.log(`Cập nhật thanh toán với id: ${id}`);
    return this.paymentsService.update(id, updatePaymentDto);
  }

  @Delete(':id')
  remove(@Param('id', new ParseUUIDPipe({ errorHttpStatusCode: 400, exceptionFactory: () => new BadRequestException('ID phải là UUID hợp lệ') })) id: string) {
