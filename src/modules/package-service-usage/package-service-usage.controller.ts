...
export class PackageServiceUsageController {
  ...
  @ApiResponse({ status: 201, description: 'Package service usage created successfully', type: CreatePackageServiceUsageDto })
  ...
  @ApiOperation({ summary: 'Get a list of all package service usage records' })
  @ApiResponse({ status: 200, description: 'Package service usage records retrieved successfully' })
  ...
  @ApiResponse({ status: 200, description: 'Package service usage retrieved successfully', type: CreatePackageServiceUsageDto })
  @ApiResponse({ status: 400, description: 'Invalid package service usage ID format' })
  @ApiResponse({ status: 404, description: 'Package service usage not found' })
  @ApiResponse({ status: 403, description: 'Forbidden: Only Staff, Manager, or Admin can perform this action' })
  @ApiParam({ name: 'id', description: 'Package service usage ID', type: String })
  @ResponseMessage('Package service usage retrieved successfully')
  async findOne(
    @Param('id', new ParseUUIDPipe({
      exceptionFactory: () => new BadRequestException('Invalid package service usage ID'),
    })) id: string,
  ) {
  ...
  @ApiResponse({ status: 200, description: 'Package service usage updated successfully', type: UpdatePackageServiceUsageDto })
  @ApiResponse({ status: 400, description: 'Invalid package service usage ID format' })
  @ApiResponse({ status: 404, description: 'Package service usage not found' })
  @ApiResponse({ status: 403, description: 'Forbidden: Only Admin or Manager can perform this action' })
  @ApiParam({ name: 'id', description: 'Package service usage ID', type: String })
  ...
  @ApiOperation({ summary: 'Soft delete a package service usage record by ID' })
  @ApiResponse({ status: 200, description: 'Package service usage deleted successfully' })
  @ApiResponse({ status: 400, description: 'Invalid package service usage ID format' })
  @ApiResponse({ status: 404, description: 'Package service usage not found' })
